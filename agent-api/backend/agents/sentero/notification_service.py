...
class EmailNotificationProvider(NotificationProvider):
    ...
    def send(self, contact: dict[str, Any], title: str, text: str, config: dict[str, Any]) -> None:
        to_email = str(contact.get("email") or config.get("test_recipient") or "").strip()
...
class NotificationService:
    ...
    def _test_error(self, dev: bool, detail: str) -> dict[str, Any]:
        response = {"ok": False, "message": "Die Testnachricht konnte nicht gesendet werden. Bitte prüfen Sie die Zugangsdaten."}
        if dev:
