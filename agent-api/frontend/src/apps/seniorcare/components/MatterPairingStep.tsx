...
import { api, type SeniorMatterCapabilities, type SeniorMatterDevice, type SeniorMatterStatus } from '@shared/api/client';
...
export function MatterPairingStep({ onSaved }: { onSaved: () => void }) {
  ...
  const [status, setStatus] = useState<SeniorMatterStatus | null>(null);
  const [device, setDevice] = useState<SeniorMatterDevice | null>(null);
  const [room, setRoom] = useState('living_room');
  const [role, setRole] = useState('presence');
  const [error, setError] = useState('');
  const [capabilities, setCapabilities] = useState<SeniorMatterCapabilities | null>(null);
  ...
  async function loadCapabilities() {
    ...
    try {
      const result = await api.seniorMatterCapabilities(devMode);
  ...
  async function startPairing(payload: string) {
    ...
    try {
      const result = await api.startSeniorMatter(clean.startsWith('MT:') ? { qr_payload: clean } : { setup_code: clean });
  ...
  async function pollStatus(id: string) {
    try {
      const nextStatus = await api.seniorMatterStatus(id, devMode);
      ...
      if (nextStatus.status === 'completed') {
        const nextDevice = await api.seniorMatterDevice(id, devMode);
  ...
  async function saveSensor() {
    ...
    try {
      await api.assignSeniorMatterDevice(commissioningId, { room, role });
...
function DevDetails({ status, device, setupCode, capabilities }: { status: SeniorMatterStatus | null; device: SeniorMatterDevice | null; setupCode: string; capabilities: SeniorMatterCapabilities | null }) {
  return (
