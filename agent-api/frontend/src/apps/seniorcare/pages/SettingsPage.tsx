...
export function SettingsPage({ activeTab }: { activeTab: SeniorCareSettingsTab }) {
  ...
  async function load() {
    try {
      const [nextStatus, nextSensors, nextChannels] = await Promise.all([
        ...
        api.seniorNotificationChannels(),
      ]);
  ...
  async function addContact() {
    ...
    try {
      ...
      await api.saveSeniorContact(payload);
  ...
  async function saveEditedContact() {
    ...
    try {
      ...
      await api.updateSeniorContact(editingContactId, payload);
  ...
  async function saveNotifications(nextNotifications = notifications) {
    try {
      await api.saveSeniorNotifications(nextNotifications);
      toast('Gespeichert');
      await load();
    } catch (err) {
  ...
  function hydrateChannelForms(nextChannels: SeniorNotificationChannel[]) {
  ...
  async function saveChannel(channel: 'email' | 'telegram' | 'whatsapp') {
    try {
      ...
      await api.saveSeniorNotificationChannel(channel, { enabled: false, config });
  ...
  async function testChannel(channel: 'email' | 'telegram' | 'whatsapp') {
    try {
      const result = await api.testSeniorNotificationChannel(channel);
      if (result.ok) {
        setError('');
...
function channelState(channels: SeniorNotificationChannel[], channel: string) {
  const item = channels.find((entry) => entry.channel === channel);
  if (!item?.configured) return 'Nicht eingerichtet';
  return item.enabled ? 'Aktiv' : 'Test erforderlich';
}

function channelAvailability(channels: SeniorNotificationChannel[]) {
