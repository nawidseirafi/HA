...
import { api, type SeniorTrustedContact } from '@shared/api/client';

export function ContactsPage() {
  const [contacts, setContacts] = useState<SeniorTrustedContact[]>([]);

  useEffect(() => {
    void api.seniorSetupStatus().then((status) => setContacts(status.trusted_contacts || [])).catch(() => undefined);
  }, []);
