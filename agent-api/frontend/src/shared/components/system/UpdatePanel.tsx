...
type Props = {
  variant?: 'personal' | 'seniorcare';
};

export function UpdatePanel({ variant = 'personal' }: Props) {
  const [status, setStatus] = useState<UpdateStatus | null>(null);
  const [adminStatus, setAdminStatus] = useState<UpdateStatus | null>(null);
  ...
  const rootClass = variant === 'seniorcare' ? 'seniorcare-update-panel' : 'panel settings-card update-panel';
  ...
  return (
    <section className={rootClass}>
      ...
      {status?.dev_mode && adminStatus && <DeveloperDetails status={adminStatus} />}
    </section>
  );
}
  ...
  return (
    <details className="update-developer-details">
      ...
      <div className="update-version-grid">
        <VersionItem label="Docker" value={version?.docker_version || '-'} />
        <VersionItem label="Home Assistant" value={version?.homeassistant_version || '-'} />
        <VersionItem label="Ollama" value={version?.ollama_version || '-'} />
