...
Der ContextService ist eine Querschnittskomponente. Home Assistant bleibt die Datenquelle fuer Sensor-, Personen-, Garagen-, Licht-, Medien-, Tuer- und Praesenzsignale. Der ContextService liest diese Signale und berechnet daraus ausschliesslich Zustaende:
...
Garagen- und Abfahrtslogik wird ueber einen `DepartureContext` bewertet. Beim Verlassen wird erst ein Beobachtungsfenster genutzt. Der ContextService verwendet die konfigurierte Person-Entity als Heimkehr-/Abfahrtssignal. Kurzabwesenheiten liefern `SHORT_AWAY` und koennen `GarageState.KEEP_OPEN` ergeben. Erst wenn das Fenster abgelaufen ist und die Person weiter abwesend ist, wird `PresenceState.AWAY` und bei offenem Tor `GarageState.READY_TO_CLOSE` berechnet. Bei Rueckkehr nach einer konfigurierbaren Nicht-Kurzabwesenheit liefert der Kontext `COMING_HOME` und `READY_TO_OPEN`.
