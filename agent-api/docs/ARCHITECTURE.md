...
Garagen- und Abfahrtslogik wird ueber einen `DepartureContext` bewertet. Beim Verlassen wird erst ein Beobachtungsfenster genutzt. Wenn ein Fahrzeug-Tracker vorhanden ist, wird er als primaeres Mobilitaetssignal genutzt; ohne Fahrzeug-Tracker darf der ContextService die Person-Entity als Heimkehr-/Abfahrtssignal verwenden. Kurzabwesenheiten liefern `SHORT_AWAY` und koennen `GarageState.KEEP_OPEN` ergeben. Erst wenn das Fenster abgelaufen ist und Person bzw. Fahrzeug weiter abwesend sind, wird `PresenceState.AWAY` und bei offenem Tor `GarageState.READY_TO_CLOSE` berechnet. Bei Rueckkehr nach einer konfigurierbaren Nicht-Kurzabwesenheit liefert der Kontext `COMING_HOME` und `READY_TO_OPEN`.
